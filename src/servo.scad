  ...
  attachable(anchor, spin, orient, [servo_long_side, servo_short_side, servo_long_side]) {
    union() {
      ...
      cube([servo_long_side, servo_short_side, servo_long_side], center=true) {
        ...
        position(TOP + RIGHT) {
          left(r1) {
            cyl(servo_thickness, r1, $fn=circle_edge, center=false)
              position(TOP)
                color([1, 1, 1]) {
          ...
          left(r1 + 6) cyl(servo_thickness, r2, $fn=circle_edge, center=false);
        }
      }
    }
    ...
    union() {
      ...
      up(4 - servo_arm_thickness)
        hull() {
          ...
          right(14.5) cyl(servo_arm_thickness, 2, $fn=circle_edge, center=false);
        }
    }
